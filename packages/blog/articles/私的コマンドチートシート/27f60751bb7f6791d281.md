...
title: 私的コマンドチートシート
created_at: 2025-04-29T14:27:31+09:00
updated_at: 2025-05-31T00:51:07+09:00
tags: [{"name":"command","versions":[]},{"name":"archLinux","versions":[]},{"name":"CheatSheet","versions":[]}]
private: false
url: https://qiita.com/takaya_maekawa/items/27f60751bb7f6791d281
likes_count: 0
---
...
## 権限
### 再帰的にリセット
以下は、カウントディレクトリ以下で、`.git`と`vendor`ディレクトリを除いたファイルやディレクトリに対して、権限をリセットするコマンド
```bash
find . -type f \( ! -path "./.git/*" ! -path "./vendor/*" \) -exec chmod 644 {} +
find . -type d \( ! -path "./.git/*" ! -path "./vendor/*" \) -exec chmod 755 {} +
```
ただ、こういうfind関数を使う場合には、それの及ぼす範囲を先に見ておきたいものだ。  
以下のコマンドで、対象のファイルが出力される。
```bash
find . -type f \( ! -path "./.git/*" ! -path "./vendor/*" \) -exec ls -l {} +
```
...
#### gitmodule
##### 追加
パス`sub`に対し、サブモジュールをインポートする例
```bash
git submodule https://github.com/<user>/<repo>.git ./sub
```
##### シンプルなインポート
`.gitmodules`に記述されているものだけをインポート。その中にサブモジュールがあっても、インポートしない。
```bash
git submodule init
```
##### 再帰的にインポート
